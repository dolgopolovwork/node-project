...
public class MasterClusterPartialFailureITCase extends AbstractContainerITCase {
    ...
    @Test
    public void testFactorPartialFailure() throws IOException, InterruptedException, TimeoutException {
        ...
        try (RpcClient client = new RpcClient("localhost", getRmqPort(rmq), RPC_REPLY_QUEUE)) {
            for (int i = 0; i < tests; i++) {
                if ((i + 1) == tests / 2) {
                    assertEquals(tests / 2 / 2, getMasterTaskMonitoring(masterFirstRegion).getExecutedTasks());
                    slaveFirstRegion.close();
                    masterFirstRegion.close();
                }
                int bits = 45;
        ...
        assertEquals(tests - (tests / 2 / 2), getMasterTaskMonitoring(masterSecondRegion).getExecutedTasks());
    }
}
