...
public class MainApplication extends NodeBenchmarkCLI {
    ...
    static {
        Container.getInstance().put(container -> {
            container.put(new ClientApplicationContainer());
    ...
    protected void benchMarkRun(CommandLine cmd) {
        ...
        new NodeBenchmark(getAppName(), getTests(cmd)).run(getSlaves(cmd), getServiceThreads(cmd), new DlpDistNodeBenchmarkPerformer(orderBitLength));
    }

    @Override
    protected String getAppName() {
        return "dlp-dist-node-benchmark";
    }
